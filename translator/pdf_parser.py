...
from domain.content import Content, ContentType, TableContent
...
def parse_pdf(pdf_file_path: str, pages: Optional[int] = None) -> Book:
    ...
    :param pages: 可选，需要翻译的前多少页。不传时，默认就是整个pdf文件
    :return:
    """
    book = Book(pdf_file_path)  # 一个pdf对应一本书
    with pdfplumber.open(pdf_file_path) as pdf:
        # pages 不能超过pdf文件中的总页数
        ...
        for pdf_page in pages_arr:
            page = Page()  # 每一页就是一个page页面

            # 从pdf的page中截取内容
            raw_text = pdf_page.extract_text()
            tables = pdf_page.extract_tables() # 还有extract_table() 返回第一个表格
            # 会把tables的内容重复提取，需要从raw_text中删除，表格中已经存在的文本
            for table in tables:
                for row in table:
                    for cell in row:
